...
pub fn initialize_workspace(app_state: Arc<AppState>, cx: &mut App) {
    ...
    cx.observe_new(|_multi_workspace: &mut MultiWorkspace, window, cx| {
        ...
        let _multi_workspace_handle = cx.entity();
    })
    .detach();
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may be due to system-wide limits on inotify instances. For troubleshooting see: https://github.com/elleryfamilia/zerminal#readme
            "#},
            e
        );
        ...
        cx.spawn(async move |_, cx| {
            if prompt.await == Ok(0) {
                cx.update(|cx| {
                    cx.open_url("https://github.com/elleryfamilia/zerminal#readme");
                    cx.quit();
                });
            }
        })
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may occur on network filesystems and WSL paths. For troubleshooting see: https://github.com/elleryfamilia/zerminal#readme
            "#},
            e
        );
        let prompt = window.prompt(
...
fn show_software_emulation_warning_if_needed(
    ...
    if specs.is_software_emulated && std::env::var("ZED_ALLOW_EMULATED_GPU").is_err() {
        let (graphics_api, docs_url, open_url) = if cfg!(target_os = "windows") {
            (
                "DirectX",
                "https://github.com/elleryfamilia/zerminal#readme",
                "https://github.com/elleryfamilia/zerminal#readme",
            )
        } else {
            (
        ...
        let message = format!(
            db::indoc! {r#"
            Zerminal uses {} for rendering and requires a compatible GPU.
...
fn initialize_panels(window: &mut Window, cx: &mut Context<Workspace>) -> Task<anyhow::Result<()>> {
    cx.spawn_in(window, async move |workspace_handle, mut cx| {
        ...
        let ai_terminal_panel =
            ai_terminal_panel::AiTerminalPanel::load(workspace_handle.clone(), &mut cx);
        let context_panel =
            context_panel::ContextPanel::load(workspace_handle.clone(), &mut cx);
        ...
        futures::join!(
            ...
            add_panel_when_ready(ai_terminal_panel, workspace_handle.clone(), cx.clone()),
            add_panel_when_ready(context_panel, workspace_handle.clone(), cx.clone()),
        );
...
fn open_about_window(cx: &mut App) {
    ...
    cx.open_window(
        WindowOptions {
            titlebar: Some(TitlebarOptions {
                title: Some("About Zerminal".into()),
                appears_transparent: true,
...
pub fn load_default_keymap(cx: &mut App) {
    ...
    if let Some(asset_path) = base_keymap.asset_path() {
        cx.bind_keys(KeymapFile::load_asset_allow_partial_failure(asset_path, cx).unwrap());
    }
...
mod tests {
    ...
    async fn test_opening_project_settings_when_excluded(cx: &mut gpui::TestAppContext) {
        ...
        eprintln!("Created project with .zerminal/settings.json containing UNIQUEVALUE");
        ...
        let original_settings = app_state
            .fs
            .load(Path::new("/root/.zerminal/settings.json"))
            .await
            .unwrap();
        ...
        let has_zed_entry =
            cx.update(|cx| worktree.read(cx).entry_for_path(rel_path(".zerminal")).is_some());
