...
pub(crate) async fn restore_or_create_workspace(
    ...
    cx.update(|cx| {
        ...
        // spawning a duplicate terminal tab.
...
// Restorable workspace helpers (`restorable_workspaces` /
// `restorable_workspace_locations`) were removed alongside the session-
// restore bypass in `restore_or_create_workspace` — Zerminal always opens
// a blank slate on launch. If recent-project UX grows to need them, the
// git history has the previous implementations.
