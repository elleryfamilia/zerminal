...
impl OpenRequest {
    pub fn parse(request: RawOpenRequest, cx: &App) -> Result<Self> {
        ...
        for url in request.urls {
            ...
            } else if let Some(action_index) = url.strip_prefix("zerminal-dock-action://") {
...
pub fn listen_for_cli_connections(opener: OpenListener) -> Result<()> {
    ...
    let sock_path = paths::data_dir().join(format!("zerminal-{}.sock", *RELEASE_CHANNEL_NAME));
...
async fn open_workspaces(
    ...
    if grouped_locations.is_empty() {
        // CLI asked for a new empty workspace (`--new`), so honor that
        // explicitly. The Quickstart first-run gate lives in
        // `restore_or_create_workspace` — the only path that lands here is an
        // intentional empty-window request.
        cx.update(|cx| {
...
mod tests {
    ...
    fn test_parse_shared_agent_thread_url(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec![format!("zerminal://agent/shared/{session_id}")],
                    ..Default::default()
                },
                cx,
            )
    ...
    fn test_parse_shared_agent_thread_url_with_invalid_uuid(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["zerminal://agent/shared/not-a-uuid".into()],
                    ..Default::default()
                },
                cx,
            )
