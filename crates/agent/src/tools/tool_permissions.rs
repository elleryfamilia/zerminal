...
/// `.zerminal/settings.json` where `.zed` is a symlink to an external directory.
///
...
/// either inside a `.zerminal/` local-settings directory or inside the global config dir.
