...
/// On Linux/FreeBSD, this is `$XDG_CONFIG_HOME/zerminal`.
/// On Windows, this is `%APPDATA%\Zerminal`.
...
/// Returns the path to the configuration directory used by Zerminal.
pub fn config_dir() -> &'static PathBuf {
    CONFIG_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            ...
            home_dir().join(".config").join("zerminal")
        }
    })
}

/// Returns the path to the data directory used by Zerminal.
pub fn data_dir() -> &'static PathBuf {
    CURRENT_DATA_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            custom_dir.clone()
        } else if cfg!(target_os = "macos") {
            home_dir().join("Library/Application Support/Zerminal")
        } else if cfg!(any(target_os = "linux", target_os = "freebsd")) {
...
pub fn state_dir() -> &'static PathBuf {
    ...
    STATE_DIR.get_or_init(|| {
        if cfg!(target_os = "macos") {
            return home_dir().join(".local").join("state").join("Zerminal");
        }
...
/// Returns the path to the temp directory used by Zerminal.
pub fn temp_dir() -> &'static PathBuf {
    ...
    TEMP_DIR.get_or_init(|| {
        ...
        home_dir().join(".cache").join("zerminal")
    })
}
...
pub fn logs_dir() -> &'static PathBuf {
    ...
    LOGS_DIR.get_or_init(|| {
        if cfg!(target_os = "macos") {
            home_dir().join("Library/Logs/Zerminal")
        } else {
            data_dir().join("logs")
        }
    })
...
/// Returns the path to the Zerminal server directory on this SSH host.
...
/// Returns the path to the `Zerminal.log` file.
pub fn log_file() -> &'static PathBuf {
    static LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    LOG_FILE.get_or_init(|| logs_dir().join("Zerminal.log"))
}

/// Returns the path to the `Zerminal.log.old` file.
pub fn old_log_file() -> &'static PathBuf {
    static OLD_LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    OLD_LOG_FILE.get_or_init(|| logs_dir().join("Zerminal.log.old"))
}
...
/// # Arguments
///
/// * `dev_mode` - If true, assumes the current working directory is the Zerminal repository.
...
///
/// This is where language servers are downloaded to for languages built-in to Zerminal.
...
///
/// This is where debug adapters are downloaded to for DAPs that are built-in to Zerminal.
...
/// Returns the relative path to a `.zerminal` folder within a project.
...
pub fn local_settings_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".zerminal/settings.json").unwrap());
    *CACHED
}
...
pub fn local_tasks_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".zerminal/tasks.json").unwrap());
    *CACHED
}
...
pub fn local_debug_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".zerminal/debug.json").unwrap());
    *CACHED
}
