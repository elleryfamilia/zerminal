...
async fn test_sharing_an_ssh_remote_project(
    ...
    worktree_a.update(cx_a, |worktree, _cx| {
        assert_eq!(
            ...
            vec![
                rel_path(".zerminal"),
                rel_path(".zerminal/settings.json"),
                rel_path("README.md"),
                rel_path("src"),
                rel_path("src/lib.rs"),
            ]
...
async fn test_ssh_remote_worktree_trust(cx_a: &mut TestAppContext, server_cx: &mut TestAppContext) {
    ...
    cx_a.read(|cx| {
        assert_eq!(
            ...
            ["...".to_string()],
            "remote .zerminal/settings.json must not sync before trust approval"
        )
    });
    ...
    cx_a.read(|cx| {
        assert_eq!(
            LanguageSettings::for_buffer(buffer_before_approval.read(cx), cx).language_servers,
            ["override-rust-analyzer".to_string()],
            "remote .zerminal/settings.json should sync after trust approval"
        )
    });
